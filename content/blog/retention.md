...
Для расчетов будем использовать синтетический dataset состоящий из двух полей (столбцов): *«user_id»* — уникальный ID пользователя; *«date»* — дата посещения продукта. Исходный dataset можно найти по [ссылке](https://zavarov.com/data/retention-dataset.csv), вот так выглядят его первые 10 строк:
