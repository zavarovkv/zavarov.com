...
- Both manually maintained homepages use `{{< latest-posts count="3" >}}` before Projects. The theme selects the latest visible posts by date in the current language automatically.
- Long posts (700+ words and at least three Markdown `##` headings) get a collapsed table of contents below the title. `toc = true` / `toc = false` overrides the threshold; pages without navigable headings never get an empty control.
- Both root and RU-specific `markup.highlight` blocks set `noClasses = false`, enabling the theme's light/dark syntax palette. Keep those settings in sync.
